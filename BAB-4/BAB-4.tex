...
Berdasarkan tabel \ref{tab:hasil_evaluasi}, terlihat sistem menunjukkan performa yang cukup baik pada metrik \textbf{Context Precision} dengan skor 0.194. Skor ini menunjukkan bahwa fitur \textit{version aware} yang
...
Nilai \textbf{Faithfulness} sebesar menunjukkan bahwa halusinasi model, sehingga jawaban yang diberikan oleh LLM juga sesuai dengan konteks yang diambil dari data vektor yang ada, bukan dari jawaban yang dibuat oleh LLM itu sendiri.
...
\begin{table}[h!]
    ...
    \begin{tabular}{|l|c|}
        ...
        Context Precision & 0.194 \\ \hline
        Context Recall & 0.500 \\ \hline
        Faithfulness & 0.250 \\ \hline
        Answer Relevancy & 0.602 \\ \hline
    \end{tabular}
\end{table}



% \section{Implementasi Antarmuka Chatbot}
% \label{sec:implementasi}

% Antarmuka \textit{chatbot} dikembangkan menggunakan [Framework UI kamu, misal: Streamlit] untuk memudahkan interaksi pengguna. Fitur utama yang ditonjolkan adalah opsi pemilihan versi dokumen sebelum memulai percakapan.

% \begin{figure}[h!]
%     \centering
%     % \includegraphics[width=0.8\textwidth]{gambar/screenshot_ui.png} 
%     % Ganti path gambar sesuai file kamu
%     \caption{Tampilan Antarmuka Chatbot dengan Pemilihan Versi}
%     \label{fig:ui_chatbot}
% \end{figure}

% Pada Gambar \ref{fig:ui_chatbot}, terlihat pengguna dapat memilih versi dokumen. Ketika pengguna bertanya mengenai [Contoh topik], sistem secara spesifik memberikan jawaban berdasarkan [Versi X], mengabaikan informasi yang bertentangan dari [Versi Y].