...
Untuk menyelesaikan permasalahan yang dirumuskan, penelitian ini mengembangkan sistem \textit{chatbot} dengan arsitektur yang mengintegrasikan metode konvensional dengan \textit{version-aware}. Metodologi penelitian dirancang untuk menangani dokumen yang memiliki riwayat perubahan (versi) secara kronologis.

\section{Desain Arsitektur Sistem}
Sistem dibangun berdasarkan arsitektur yang memisahkan antara komponen standar RAG dengan version aware. Gambaran umum arsitektur sistem dapat dilihat pada Gambar \ref{fig:rag_architecture}.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{BAB-3/rev2-RAG-diagram.png}
    \caption{Arsitektur RAG (Conventional vs Version Aware)}
...
Tahap ini dapat dilihat pada gambar \ref{fig:rag_architecture}. Proses ini bertujuan mengubah dokumen tidak terstruktur menjadi format yang dapat dipahami oleh mesin dengan menyertakan konteks versi.
...
Tahap ini ditunjukkan pada gambar \ref{fig:rag_architecture}, yang menjelaskan bagaimana sistem merespons masukan pengguna.
...
Untuk mengukur efektivitas arsitektur \textit{Version-Aware} ini dibandingkan dengan RAG Konvensional, maka dilakukan evaluasi menggunakan metrik RAGAS (\textit{Retrieval Augmented Generation Assessment}).
